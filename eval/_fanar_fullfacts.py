# Real Fanar full-facts Arabic-LJP run (the sweep's best config, but persisting
...
cases = json.load(open(D+"ljp_blind_full.json", encoding="utf-8"))
...
json.dump(out, open(D+"ljp_fanar_full_out.json","w"), ensure_ascii=False, indent=2)
...
print(f"Fanar full-facts (corrected key): {acc:.0%}  errors={dict(errs)}")
