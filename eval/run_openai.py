#!/usr/bin/env python3
"""Run both eval tracks against OpenAI (openai-chat) via its OpenAI-compatible
API. Same blind inputs as Fanar/Claude; Arabic-LJP uses the FULL untruncated
facts (to match the fair head-to-head). Writes *_openai_out.json and scores
against the held-out keys.

Env: OPENAI_API_KEY (required), OPENAI_MODEL (default openai-chat).
"""
...
def chat(prompt, mt=1024):
    ...
    if MODEL.startswith(("gpt-5", "o1", "o3", "o4")):  # reasoning models
        payload["max_completion_tokens"] = max(mt, 6000)  # room for reasoning tokens
    else:
...
if __name__ == "__main__":
    print(f"model: {MODEL}")
    rtf = run("sample_blind.json", "classify_openai_out.json", ["routine", "non_routine"], rtf_prompt)
    score(rtf, "sample_key.json", ["routine", "non_routine"], "RTF conformance")
    ljp = run("ljp_blind_full.json", "ljp_openai_out.json", ["accept", "reject", "route"], ljp_prompt)
